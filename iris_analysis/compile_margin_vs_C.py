"""
Compile margin vs C summary for Iris pairwise runs
- Reads `results/iris_pairwise_C_study_summary.csv` (generated by `pairwise_C_analysis.py`)
  or any CSV in results matching the pattern and assembles a single summary CSV
- For each pair it computes the margin trend across C (slope of margin vs log10(C))
  and adds an explanation column per row describing observed behavior.
- Outputs `results/iris_margin_C_compilation.csv` and a JSON copy.
"""
import os
import sys
import math
...
# Prefer the main summary file created by pairwise_C_analysis.py
candidates = [
...
for p in candidates:
    if os.path.exists(p):
        found = p
        break
# if not found, search for any csv in results and try to assemble
if found is None:
    files = glob.glob(os.path.join(RESULTS_DIR, '*.csv'))
    if not files:
        print('No CSV files found in', RESULTS_DIR)
        sys.exit(1)
    # try to find one that contains 'pair' and 'C' columns
    for f in files:
        try:
...
# compute per-pair trend (slope of margin vs log10(C))
trend_map = {}
...
# add trend and explanation per row
explanations = []
